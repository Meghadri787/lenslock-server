...
class BucketService {
    ...
    async getBuckets() {
        const buckets = await Buckets.find().populate("photographer", "name email");
        return buckets;
    }
    ...
    async generateQRCode(id, userId) {
        ...
        if (bucket.photographer.toString() !== userId) {
            throw new Error("Not authorized to generate QR code for this bucket");
        }
    ...
    async requestBucketAccess(id, userId) {
        ...
        if (bucket.accessList.some(access => access.user.toString() === userId)) {
            throw new Error("You already have access to this bucket");
        }

        // Check if user already has a pending request
        if (bucket.accessRequests.some(request => request.user.toString() === userId)) {
    ...
    async respondToAccessRequest(id, requestId, response, userId) {
        ...
        if (response === "accept") {
            ...
            throw new Error("Invalid response. Must be either 'accept' or 'reject'");
        }
